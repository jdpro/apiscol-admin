(function ($) {
	$.widget("ui.rcarousel", {
		_create: function() {
            ...
            // check if user set options before init:
			// $('element').rcarousel({with: "foo", visible: 3});
            // in above example exception will be thrown because 'with' should
			// be a number!
            this._checkOptionsValidity( this.options );

			// for every carousel create a data object and keeps it in the
			// element
					...
					switch ( key ) {
						...
						case "step":
							if ( !value || typeof value !== "number" || value <= 0 || (Math.ceil(value) - value > 0) ) {
								...
								// for example for visible: 3 the following
								// array of values for 'step' is valid
								// 3 <= step >= 1 by 1 ==> [1,2,3]
						...
						case "width":
							// width & height is defined by default so you can
							// omit them to some extent
							if ( !value || typeof value !== "number" || value <= 0 || Math.ceil(value) - value > 0 ) {
		...
		_generatePages: function() {
			...
			function _paginate() {
				...
				// we start from the 2nd page (1st page has been already
				// initiated)
				while ( !_islastPage(data.pages[data.pages.length - 1]) || _isBeginning ) {
					...
					// when we run ouf of elements (_end - _start < _visible) we
					// must add the difference at the begining
					// in our example the 3rd page is [FGHIJ] and J element is
					// added in the second step
					// first we add [FGHI] as old elements
					// we must assure that we have always ‘visible’ (5 in our
					// example) elements
		...
		_goToNextPage: function( by ) {
			...
			// we compare the same
			// pages, that is, the first pages from 5 elements and the first
			// from
