...
vCard = {
	...
	SingletonMethods : {
		to_html : function() {
			...
			if (this.photo) {
				output += '<img class="photo" src="data:image/png;base64,'
						+ this.photo['base64'][0] + '" />';
			}
			//HACK
			if (this.fn) {
			...
			for (type in this.adr) {
				for (n in this.adr[type]) {
					...
					adr_fields = [ 'post-office-box', 'extended-address',
			...
			for (type in this.email) {
				for (n in this.email[type]) {
					...
					output += '<span class="type">' + type + '</span>';
					output += '<a class="value" href="mailto:' + value + '">'
							+ value + '</a>';
					output += '</span>';
				}
			}
			...
			for (type in this.url) {
				for (n in this.url[type]) {
					...
					output += '<span class="type">' + type + '</span>';
					output += '<a class="value" href="' + value + '">' + value
							+ '</a>';
					output += '</span>';
				}
			}
