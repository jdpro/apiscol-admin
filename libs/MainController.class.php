...
class MainController {
	...
	public function handleSyncRequest() {
		...
		$this->secundaryController->getView()->build();
	...
	public function getCurrentView() {
		return $this->secundaryController->getView()->toHTML();
	}
	...
	public function setErrorMessage($public, $private='') {
		if($this->errorMessages==null)
			$this->errorMessages=array("public"  => '',
					"private" => '');
	...
	public static function xmlErrorMessage($message, $code=null, $intro=null) {
		...
		$messageElem = $root->appendChild(
				$xmlDoc->createElement("message"));
		$messageElem->appendChild(
				$xmlDoc->createTextNode($message));
		if(!is_null($code)) {
			$codeElem = $root->appendChild(
					$xmlDoc->createElement("code"));
			$codeElem->appendChild(
					$xmlDoc->createTextNode($code));
		}

		if(!is_null($intro)) {
			$introElem = $root->appendChild(
					$xmlDoc->createElement("intro"));
			$introElem->appendChild(
					$xmlDoc->createTextNode($intro));
		}
