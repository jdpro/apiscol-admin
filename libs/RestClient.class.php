...
class RestClient
{
	...
	{
		$response=$this->_launch($this->_makeUrl($pParams),
				$this->_createContext('GET', null, "Accept: application/xml\r\n"."Cache-Control: no-cache\r\n"));
		if($response==false) {
	...
	public function post ($pPostParams=array(), $pGetParams = array(), $accept=null, $contentType=null, $ifMatch=null, $authorization=null)
	{
		...
		$response= $this->_launch($this->_makeUrl($pGetParams),
				$this->_createContext('POST', $pPostParams, $headers));
...
public function postMultipartWithFile ( $pPostParams=array(), $name, $file, $accept=null, $ifMatch=null, $authorization=null)
	{
		...
		$response= $this->_launch($this->_makeUrl(array()),
	...
	public function put($pContent = array(), $pGetParams = array(), $accept=null, $contentType=null, $ifMatch=null, $authorization=null)
	{
		...
		$headers.="Authorization: ".$authorization."\r\n";
		$response = $this->_launch($this->_makeUrl($pGetParams),
				$this->_createContext('PUT', $pContent, $headers));
		$this->renewAuthorization($response);
		return $response;
	}
	...
	public function delete($pContent = array(), $pGetParams = array(), $accept=null, $contentType=null, $ifMatch=null, $authorization=null)
	{
		...
		$headers.="Authorization: ".$authorization."\r\n";
		$response = $this->_launch($this->_makeUrl($pGetParams),
				$this->_createContext('DELETE', $pContent, $headers));
		$this->renewAuthorization($response);
		return $response;
	}
	...
	public function askAuthorisationToken() {
		$response=$this->setUrl($this->editUrl)->post(array(),array(), "application/xml",
				"application/x-www-form-urlencoded", null, $this->login.$this->pass);
		$this->renewAuthorization($response);
		if($this->nonce==null)
	...
	protected function _launch ($pUrl, $context)
	{
		...
		fclose($stream);
		return array('error'=>FALSE, 'content'=>$content, 'header'=>$header);
